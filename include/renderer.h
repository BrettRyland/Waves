...
namespace Waves {
	namespace Renderer {
		...
		struct OpenGL_Resources {
			Waves::Mesh::surface_mesh surface;
			std::vector<Waves::Mesh::surface_vertex> surface_vertex_array;
			...
			glm::vec3 eye{ 0.0f, -3.0f, 6.0f }, centre{ 0.0f, 0.0f, 0.0f }, up = { 0.0f, 0.0f, 1.0f };
		...
		void render_mesh(const Waves::Mesh::surface_mesh& mesh);

		// Helper function for scaling the z-axis to make for better viewing.
		glm::mat4 rescale_z(float hint = 1.0f);
	}
}