	...
	*/
	...
	public:
		...
		*/
		...
		// integrator
		void change_boundary_conditions(int bc) { m_integrator.change_boundary_conditions(bc); m_mesh.generate_surface_mesh(m_integrator); } ///< Change the boundary conditions and generate the static components of a new surface mesh. The vertex buffer and mesh will need to be re-connected and updated after calling this.
		void change_initial_conditions(int ic) { m_integrator.change_initial_conditions(ic); m_mesh.update_surface_mesh_CL(m_integrator); } ///< Change the initial conditions and update the surface mesh (dynamic components)
		void set_artificial_dissipation(double d) { m_integrator.set_artificial_dissipation(static_cast<integrator_precision>(d)); } ///< Set the artificial dissipation in the integrator
		void change_time_step(double dt) { m_integrator.change_time_step(static_cast<integrator_precision>(dt)); } ///< Change the time step of the integrator
		void reverse_time() { m_integrator.change_time_step(-m_integrator.get_time_step()); m_integrator.half_step(); m_integrator.half_step(); } ///< Take 2 half-steps to set up the integrator for going backwards
		// mesh
		void connect_mesh_with_vertex_buffer(QOpenGLBuffer & vertex_buffer) { m_mesh.configure_mesh_update_kernel(m_integrator, vertex_buffer); m_mesh.update_surface_mesh_CL(m_integrator); } ///< Connect the mesh with the OpenGL vertex buffer
		void update_surface_mesh() { m_mesh.update_surface_mesh_CL(m_integrator); } ///< Update the surface mesh ("dynamic" components

		/** Accessors
		@{*/
		inline bool const is_paused() const { return m_paused; } ///< Check our paused state
		auto const get_steps_taken() { auto steps_taken = m_step_counter; m_step_counter = 0; return steps_taken; } ///< Returns the number of steps taken since the last time this function was called.
