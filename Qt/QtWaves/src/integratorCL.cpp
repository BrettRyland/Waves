...
namespace Waves
{
	...
	void Integrator::initialise(int rx, int ry, integrator_precision dt, int ic, int bc, boost::compute::context &shared_context, boost::compute::command_queue &queue)
	{
		...
		height_scale = static_cast<integrator_precision>(1);
		wave_speed_scale = static_cast<integrator_precision>(1);
	...
	*/
	void Integrator::configure_kernels_dynamic_vars(std::vector<integrator_precision> const &host_U, std::vector<integrator_precision> const &host_V)
	{
		...
		kernel_step_V.set_args(V.get_buffer(), masks.get_buffer(), U.get_buffer(), U_xx.get_buffer(), U_yy.get_buffer(), cell_type_x.get_buffer(), cell_type_y.get_buffer(), cell_type_count, nodes_per_cell, step_size_time, wave_speed * wave_speed_scale);
	...
	*/
	void Integrator::change_initial_conditions(int ic)
	{ // Change the initial conditions
		...
		case 2:
		{ // Continuous spectrum of frequencies, but limited to localised hump in the middle
		...
		case 3:
		{ // Continuous spectrum of frequencies, but limited to 2 localised humps.
		...
		case 4:
		{ // Continuous spectrum of frequencies, but limited to an off-centre localised hump.
			// u=exp(f(x,y))-1, where f(x,y)=c*(1-x^2/bx^2-y^2/by^2)
			// note: if c/bx^2 or c/by^2 is too large then the integrator will break due to a CFL condition!
			integrator_precision bx = static_cast<integrator_precision>(0.3), by = static_cast<integrator_precision>(0.3), a = static_cast<integrator_precision>(1.5), x0 = sqrt(half) * half, y0 = x0;
			for (cl_uint c = 0; c < num_cells; ++c)
		...
		case 5:
		{ // Continuous spectrum of frequencies in the y-direction
		...
		time = zero;
		if (height_scale != static_cast<integrator_precision>(1))
		{
			for (cl_uint c = 0; c < num_cells; ++c)
				for (unsigned int j = 0; j < stages_y; ++j)
					for (unsigned int i = 0; i < stages_x; ++i)
						host_U[c * nodes_per_cell + j * stages_x + i] *= height_scale;
		}
	...
	std::tuple<std::vector<cl_uint>, std::vector<cl_uint>, std::vector<cl_uint>> Integrator::find_neighbours(std::vector<integrator_precision> const &position_information)
	{
		...
		for (cl_uint c = 0; c < num_cells; ++c)
		{
			...
			for (; i != -1; --i)
			{																																																	// continue looking for top neighbour
			...
			i = c + 1;
			for (; i != static_cast<int>(num_cells); ++i)
			...
			for (; i != static_cast<int>(num_cells); ++i)
			{																																																	// continue looking for bottom neighbour
	...
	*/
	...
	{ // Change the boundary conditons
		...
		case 2:
		{ // Circle with Dirichlet boundaries.
			/*
		...
		case 3:
		{ // Circle with a cusp, Dirichlet boundaries
			/*
		...
		case 4:
		{ // 2 intersecting circles, Dirichlet boundaries
			int n = 300;
			step_size_x = static_cast<integrator_precision>(0.1);
			step_size_y = static_cast<integrator_precision>(0.1);
			wave_speed = static_cast<integrator_precision>(10.0);
		...
		case 5:
		{ // Double slit in a square with some periodic and some Dirichlet boundaries
